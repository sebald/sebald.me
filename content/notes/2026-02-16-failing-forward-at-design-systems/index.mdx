...
We needed to pivot. We had assumed teams were just waiting for us to hand them something better. They weren't. We met with them to understand their perspective.

The result was counterintuitive: we disguised our new design system to look exactly like the legacy app. We brought it into the legacy codebase and matched the visual style so that anything built with the new components was indistinguishable from the old code. Under the hood, developers were working with modern tools. On the surface, nothing changed.
