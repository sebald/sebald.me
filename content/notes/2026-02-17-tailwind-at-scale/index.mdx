...
description: As your project grows, utility-first CSS runs into a repetition problem. Tailwind v4 gives you a clean way out by combining @utility and @apply into custom utilities that behave just like built-in ones.
topics: [css, tailwind, design-systems]
date: 2026-02-17
---

Keeping CSS maintainable is one of those problems that never fully goes away, and everyone seems to have a different idea about how to solve it. Some swear by strict naming conventions like BEM. Others go all in on encapsulation with CSS Modules or CSS-in-JS. And then there's the utility-first camp, which says: stop naming things, just apply small, single-purpose classes directly in your markup.

At this point, utility-first CSS has won. Tailwind is everywhere.

If you ask critics of utility-first CSS what bothers them most, the answer is almost always the same: repetition. It's not a problem you notice on day one. But as your project grows, certain visual patterns start repeating across your codebase. The same cluster of utility classes, copy-pasted into a dozen places. Then the design changes and you're hunting down every instance.

Components help, but not every shared pattern is a component. A focus state isn't a component. A disabled appearance isn't a component. The base look of an input isn't a component. These are cross-cutting concerns, and utility-first CSS has historically had no good answer for them.

Tailwind v4 changes that. By combining two features, `@utility` and `@apply`, you can extract visual patterns into custom utilities that behave just like Tailwind's built-in ones.
