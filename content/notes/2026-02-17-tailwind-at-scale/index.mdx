...
Here's where it gets interesting. Put `@apply{:css}` inside `@utility{:css}` and you get the best of both:
...
Tailwind's built-in utilities already embrace this pattern. `@utility{:css}` + `@apply{:css}` just lets you do the same thing.
