...
One common critique of Tailwind is that it doesn't scale well. It's not a problem you notice on day one. But as your project grows, you start copy-pasting the same cluster of utility classes into a dozen places. Then the design changes and you're hunting down every instance.

Components are one way to deal with this. If you keep repeating the same classes, wrap them in a component and reuse that instead. But not every shared pattern is a component. A focus state isn't a component. A disabled appearance isn't a component. The base look of an input isn't a component.

These are cross-cutting concerns, and the usual answer is to reach for something like BEM or CSS Modules to bring back structure.

But you don't have to leave Tailwind's system to solve this. With Tailwind 4, you can combine two features, `@utility{:css}` and `@apply{:css}`, to bring structure to your styles without giving up anything Tailwind offers.

## A quick recap

If you're already deep in Tailwind 4, feel free to skip ahead. But for the sake of having a shared vocabulary, let's quickly cover the two building blocks.
