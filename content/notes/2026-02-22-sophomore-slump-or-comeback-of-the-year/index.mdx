...
When you build a design system inside a legacy product, you think the work is about how the product looks. How it behaves. The shape of every screen. You're doing something else the whole time. The actual goal is to enable colleagues and the company to build better products. But the shape of that work is hidden, even from where you're standing.
