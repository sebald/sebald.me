...
And then we started running into its limits.

The design system could do more than the old interface allowed, but nobody could see that because we had made it invisible on purpose. The gap kept growing. Meanwhile, signals were accumulating that the dated UI was costing us. Conversations with potential customers. Feedback from sales. The occasional lost deal where the product worked well enough but didn't look like it did.
...
We hadn't been idle. By the time the signals started accumulating, we had already done the quiet work of preparation. We had agreed on a visual direction — modern, simple, unobtrusive. Nothing that would shock. A safe bet that could earn trust before it asked for anything. We had mapped out which conversations needed to happen with which people, and roughly in what order. We had run through what could go wrong. Not a formal risk assessment, more like a pre-mortem: if this fails six months from now, what was the reason?
