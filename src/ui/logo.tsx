...
export const Logo = ({ size = 32 }: LogoProps) => (
  ...
  >
    ...
    <LogoIcon
      size={size}
      className="text-mist-400 mt-0.5 [grid-area:1/1] group-hover:hidden"
    />

    {/* Glitch layers */}
    <LogoIcon
      size={size}
      className="group-hover:animate-glitch-shift-slow mt-0.5 hidden text-rose-500 opacity-70 mix-blend-screen [grid-area:1/1] group-hover:block"
      style={{ clipPath: 'inset(10% 0 80% 0)' }}
    />
    <LogoIcon
      size={size}
      className="group-hover:animate-glitch-shift-fast mt-0.5 hidden text-lime-400 opacity-70 mix-blend-screen [grid-area:1/1] group-hover:block"
